          ...
          <div className={`flex items-center ${isSidebarOpen ? 'space-x-3 bg-white/5 p-3 rounded-2xl' : ''}`}>
            ...
            {isSidebarOpen && (
              <div className="overflow-hidden">
                ...
                <p className="text-xs text-gray-400 truncate">{user?.role === 'ADMIN' ? 'Quản trị viên' : 'Nhân viên vận hành'}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#f8f9fc] relative">
        ...
        <header className="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-8 sticky top-0 z-10">
          ...
          <div className="flex items-center space-x-6">
            <button className="relative p-2 text-gray-400 hover:text-sky-600 hover:bg-sky-50 rounded-xl transition-all">
